...
//! *placement* of each optional field. A field is OMITTED when the
//! flag is absent (no JSON null, no empty string); the on-wire order
//! is `type, data, source, parentEventId?, traceId?, dedupeKey?`.
//!
//! Subjects: the wire envelope retains a `subject` slot for
//! forward-compat but neither the CLI nor the MCP server populates
//! it. `publish_omits_subject_unconditionally` pins that.
...
#[tokio::test]
async fn publish_omits_subject_unconditionally() {
    // Subjects are no longer modelled on the client side. The wire
    // envelope's `subject` slot must NEVER appear on a CLI-published
    // event today; a regression that re-introduces subject plumbing
    // would land here.
    ...
    assert!(
        ...
        "subject must be omitted from every CLI-published envelope; got {env}",
    );
}
...
async fn publish_wire_field_order_with_all_optionals_set() {
    ...
    let home = tempdir();
    let mut args = base_args();
    args.extend(["--parent", "evt_p", "--trace", "tr", "--dedupe-key", "dk"]);
    let out = run_nt_publish(&server.uri(), Some("nt_push_token"), home.path(), &args).await;
    assert_eq!(out.code, 0, "unexpected failure: stderr={:?}", out.stderr);
    ...
    assert!(
        t_type < t_data && t_data < t_src && t_src < t_par && t_par < t_trc && t_trc < t_dk,
        "wire order must be type, data, source, parentEventId, traceId, dedupeKey — got {body}",
    );
}
