...
use nt_schemas::{bundle_version, known_type_ids, validate, ValidationIssue};
...
fn missing_required_field_is_reported_with_field_path() {
    ...
    assert!(
        !issues.is_empty(),
        "missing required taskId must produce issues"
    );
    assert!(
        issues
            .iter()
            .any(|i| i.path == "taskId" || i.message.contains("taskId")),
...
fn wrong_field_type_is_reported() {
    ...
    assert!(
        !issues.is_empty(),
        "wrong type for durationMs must produce issues"
    );
    assert!(
...
fn multiple_violations_surface_as_multiple_issues() {
    ...
    let paths: std::collections::HashSet<&str> = issues.iter().map(|i| i.path.as_str()).collect();
    assert!(
